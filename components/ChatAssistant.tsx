  ...
  return (
    <>
      ...
      <div className={`fixed bottom-6 right-6 w-80 sm:w-[400px] bg-black/95 backdrop-blur-2xl rounded-[2.5rem] shadow-[0_30px_100px_rgba(0,0,0,0.8)] border border-gold/30 flex flex-col overflow-hidden transition-all duration-500 z-[60] ${isOpen ? 'h-[600px] opacity-100 scale-100 translate-y-0' : 'h-0 opacity-0 scale-95 translate-y-10 pointer-events-none'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-black/40">
          ...
          <div className="flex justify-start">
            <div className="max-w-[85%] rounded-3xl px-5 py-4 text-sm leading-relaxed tracking-wide bg-white/5 text-white/80 backdrop-blur-md border border-white/10 rounded-bl-none">
              ...
              如需諮詢房市趨勢、預算配置建議，或是預約專業顧問，請點擊下方按鈕加入 LINE，由專人為您服務！
            </div>
          </div>
          ...
          <div className="flex justify-center mt-8">
            <a 
              ...
              className="flex items-center gap-3 bg-gold text-black font-bold px-8 py-4 rounded-3xl shadow-lg shadow-gold/20 hover:scale-105 transition-all"
            >
        ...
        {/* 移除输入框和发送按钮（因为不需要 AI 对话） */}
        <div className="p-6 border-t border-gold/20 bg-black text-center text-white/50 text-sm">
          點擊上方按鈕，立即與專業顧問對話
        </div>
      </div>
    </>
